...
class PDFReport(FPDF):
    ...
    def score_box(self, score, label):
        ...
        self.cell(0, 12, f'  {label}: {score}/10', fill=True, new_x='LMARGIN', new_y='NEXT')
        self.set_text_color(0, 0, 0)
        self.ln(5)
...
def generate_review_pdf(data):
    ...
    for s in scores:
        ...
        fl = next((r for r in lint_results if r.get("filename") == s.get("filename")), {})
        fl_logic = next((r for r in logic_results if r.get("filename") == s.get("filename")), {})
        fl_fixes = next((r for r in fix_suggestions if r.get("filename") == s.get("filename")), {})
    ...
    for s in scores:
        ...
        if justification:
            ...
            pdf.cell(0, 6, f'Rating: {justification[:100]}', new_x='LMARGIN', new_y='NEXT')
        pdf.ln(3)
        ...
        if lint_issues:
            ...
            pdf.cell(0, 7, f'LINT ISSUES ({len(lint_issues)}):', new_x='LMARGIN', new_y='NEXT')
            pdf.set_text_color(0, 0, 0)
            ...
            for issue in lint_issues:
                ...
                if len(msg) > 80:
                    msg = msg[:80] + "..."
                pdf.cell(0, 5, f'  [{sev}] Line {line}: {msg}', new_x='LMARGIN', new_y='NEXT')
                ...
                if suggestion:
                    ...
                    pdf.cell(0, 4, f'    Suggestion: {sug_clean}', new_x='LMARGIN', new_y='NEXT')
                    pdf.set_text_color(0, 0, 0)
            pdf.ln(2)
        ...
        if fixes:
            ...
            pdf.cell(0, 7, f'SUGGESTED FIXES ({len(fixes)}):', new_x='LMARGIN', new_y='NEXT')
            pdf.set_text_color(0, 0, 0)
            ...
            for fix in fixes:
                ...
                if orig:
                    ...
                    pdf.cell(0, 4, f'    Before: {orig_clean}', new_x='LMARGIN', new_y='NEXT')
                if fixed:
                    pdf.set_font('courier', '', 8)
                    pdf.set_text_color(39, 174, 96)
                    ...
                    pdf.cell(0, 4, f'    After:  {fixed_clean}', new_x='LMARGIN', new_y='NEXT')
                    pdf.set_text_color(0, 0, 0)
                ...
                if expl:
                    ...
                    pdf.cell(0, 4, f'    Why: {expl_clean}', new_x='LMARGIN', new_y='NEXT')
                    pdf.set_text_color(0, 0, 0)
            pdf.ln(2)
